...
var orb_n : int = 0;
var per_orb_destination : Array[int] = [];
...
func release_frontmost_orb() -> void:
	...
	spawn_output.emit(orb, per_orb_destination.back());
	per_orb_progress.pop_back();
	per_orb_destination.pop_back();
...
func update_orb_position(orb_idx: int, progress: float) -> void:
	...
	if progress < 0.5:
		...
		var dest := outputs[per_orb_destination[orb_idx]];
		var end := Vector2(dest) * pos_mult * rev_mult * GameState.gameplay_scale;
